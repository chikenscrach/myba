var del_mail_default = {
  ...
  async run({ page, shared, params, logger }) {
    ...
    while (hasNextPage && mails.length < 1e4) {
      logger.log(`\u5DF2\u641C\u96C6 ${mails.length} \u5C01\u7B26\u5408\u689D\u4EF6\u7684\u7AD9\u5167\u4FE1`);
	    const responsePromise = page.waitForResponse("https://mailbox.gamer.com.tw/ajax/inboxList.php");
      await page.waitForTimeout(100);
	    const clickPromise = page.click(".next");
      await Promise.all([responsePromise, clickPromise]);
	    const nextPageLink = await page.$(".next.no");
	  if (nextPageLink) {
		  hasNextPage = false;
	  }
      mails = [
